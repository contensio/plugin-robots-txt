<a href="{{ route('contensio-robots-txt.settings') }}" class="group flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-5 hover:shadow-sm transition-all">
    <div class="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center shrink-0">
        <svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
    </div>
    <div>
        <h3 class="text-sm font-semibold text-gray-900 group-hover:text-ember-600 transition-colors">Robots.txt Editor</h3>
        <p class="text-xs text-gray-500 mt-0.5">Manage your site's robots.txt from the admin.</p>
    </div>
</a>
